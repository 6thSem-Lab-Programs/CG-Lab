...
void myReshape(int w, int h) {
	...
	double wh = 1.0;
	if(w<=h)
		glOrtho(-wh*64/48,wh*64/48,-wh*(GLfloat)h/(GLfloat)w,wh*(GLfloat)h/(GLfloat)w,0.1,100);
	else
		glOrtho(-wh*64/48*(GLfloat)w/(GLfloat)h ,wh*64/48*(GLfloat)w/(GLfloat)h ,-wh,wh,0.1,100);
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glutPostRedisplay();
}
